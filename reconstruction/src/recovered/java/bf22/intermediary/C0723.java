package bf22.intermediary;

import java.awt.Color;
import java.io.IOException;
import java.io.Serializable;
import java.text.DateFormat;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.Collections;
import java.util.Comparator;
import java.util.Random;
import javax.swing.JOptionPane;
import mod.recovered.model.Club;
import mod.recovered.model.Coach;
import mod.recovered.model.Player;
import mod.recovered.save.SavedGameInfo;

public class C0723 implements Serializable {
   private static final long serialVersionUID = 1L;
   private SavedGameInfo ad = new SavedGameInfo();
   private int ae = 1;
   private int rM = 2022;
   private int af = 1;
   private ArrayList ag = new ArrayList();
   private ArrayList ah = new ArrayList();
   private ArrayList ai = new ArrayList();
   private ArrayList aj = new ArrayList();
   private ArrayList ak = new ArrayList();
   private ArrayList al = new ArrayList();
   private ArrayList am = new ArrayList();
   private ArrayList an = null;
   private ArrayList ao = new ArrayList();
   private ArrayList ap = new ArrayList();
   private ArrayList aq = new ArrayList();
   private ArrayList ar = new ArrayList();
   private ArrayList as = new ArrayList();
   private ArrayList at = new ArrayList();
   private ArrayList au = new ArrayList();
   private ArrayList av = new ArrayList();
   private ArrayList aw = new ArrayList();
   private ArrayList ax = new ArrayList();
   private ArrayList ay = new ArrayList();
   private ArrayList az = new ArrayList();
   private C0923 aA = null;
   private C0958 aB = null;
   private C0954 aC = null;
   private C0956 aD = null;
   private C0957 aE = null;
   private C0959 aF = null;
   private C0961 aG = null;
   private C0937 aH = new C0937();
   private C0934 Gy = new C0934();
   private C0927 aI = null;
   private C0926 aJ = null;
   private C0953 aK = null;
   private C0935 GC = null;
   private C0936 GI = null;
   private C0933 IX = null;
   private C0950 aL = null;
   private C0948 aM = null;
   private C0945 aN = null;
   private C0946 aO = null;
   private C0949 aP = null;
   private C0947 aQ = null;
   private C0940 aR = null;
   private C0952 aS = null;
   private C0939 aT = null;
   private C0941 aU = null;
   private C0944 aV = null;
   private C0943 aW = null;
   private C0960 aX = null;
   private C0932 IY = null;
   private C0930 aY = null;
   private C0928[] aZ = new C0928[4];
   private boolean[] jogaRegionaisTodos = new boolean[]{false, false, true, false};
   private boolean jogaRegionais = true;
   private boolean jogaEstadual = true;
   private boolean usaGrupoPadraoEstadual = true;
   private boolean jogaIntClubes = true;
   private boolean ba = false;
   private boolean bb = false;
   private boolean salarioMensal = true;
   private boolean habilidadeIndividual = true;
   private boolean jogaIntano1 = true;
   private boolean gruposIntPadrao = true;
   private boolean jogaSelecoesAll = true;
   private boolean[] verJint = new boolean[]{
      true, true, false, false, false, false, true, true, true, true, true, true, true, true, true, false, true, true, true, true, true
   };
   private boolean bc = false;
   private boolean bd = true;
   private int be = -1;
   private int bf = -1;
   private int bg = -1;
   private int bh = 0;
   private int autoSalvar = 0;
   private int velocidade = 3;
   private int velocidadeNH = 4;
   private int verDecisaoPenNaoHumano = 0;
   private int verMudancaTecnicos = 1;
   private int verJanelaSubs = 1;
   private int avisoTerminoContrato = 0;
   private boolean ignoraEstadual = true;
   private boolean autoRenovaContrato = true;
   private boolean usaCorPlacar = true;
   private int corPlacar = 0;
   private boolean usaSons = true;
   private boolean negritoCasa = true;
   private boolean usaCoresLista = true;
   private transient Color[] coresLista = null;
   private boolean verEstaduaisAgrupados = false;
   private boolean ignoraLigas = true;
   private boolean bi = false;
   private boolean verLeiloes = false;
   private String bj = null;
   public boolean bk = false;
   private int bl = 1;
   private int bm = 3;
   private int bn = 3;
   private int IZ = 4;
   private int bo = 2;
   private int bp = 2;
   private int bq = 4;
   private int br = 2;
   private int Ja = 2;
   private int Jb = 1;
   public boolean bs = true;
   private int bt = -1;
   private ArrayList bu = new ArrayList();
   private static transient Club X = null;
   private static transient C0675 bv = null;
   private transient ArrayList bw = new ArrayList();
   public transient ArrayList bx = null;
   public transient ArrayList by = null;
   public transient ArrayList bz = null;
   public transient ArrayList bA = null;
   public transient ArrayList bB = null;
   public transient ArrayList bC;
   public transient int[] bD;
   public transient int[] bE;
   public transient ArrayList bF;
   public transient ArrayList bG;
   public transient boolean bH = false;
   public transient boolean bI = false;
   public transient int bJ = 0;
   public transient boolean bK = false;
   public transient boolean bL = false;
   public transient boolean bM = false;
   public transient boolean bN = false;
   public transient boolean Jc = false;
   public static Comparator bO = new C0724();
   private String[] bP = new String[]{"#949449", "#B3B364", "#C0C081", "#D1D1A3", "#DEDE9E"};

   public C0723() {
      if (C0745.vM() != null) {
         this.setVerJint(C0745.vM().getVerJint());
         this.setJogaIntano1(C0745.vM().isJogaIntano1());
         this.setGruposIntPadrao(C0745.vM().isGruposIntPadrao());
         this.setJogaIntClubes(C0745.vM().isJogaIntClubes());
         this.setJogaSelecoesAll(C0745.vM().isJogaSelecoesAll());
         this.setJogaEstadual(C0745.vM().isJogaEstadual());
         this.setJogaRegionais(C0745.vM().isJogaRegionais());
         this.setSalarioMensal(C0745.vM().isSalarioMensal());
         this.setHabilidadeIndividual(C0745.vM().isHabilidadeIndividual());
         this.setIgnoraEstadual(C0745.vM().isIgnoraEstadual());
         this.setUsaGrupoPadraoEstadual(C0745.vM().isUsaGrupoPadraoEstadual());
         this.setAutoSalvar(C0745.vM().getAutoSalvar());
         this.setVelocidade(C0745.vM().getVelocidade());
         this.setVelocidadeNH(C0745.vM().getVelocidadeNH());
         this.setVerDecisaoPenNaoHumano(C0745.vM().getVerDecisaoPenNaoHumano());
         this.setVerMudancaTecnicos(C0745.vM().getVerMudancaTecnicos());
         this.setVerJanelaSubs(C0745.vM().getVerJanelaSubs());
         this.setAvisoTerminoContrato(C0745.vM().getAvisoTerminoContrato());
         this.setIgnoraLigas(C0745.vM().isIgnoraLigas());
         this.setAutoRenovaContrato(C0745.vM().isAutoRenovaContrato());
         this.setUsaCorPlacar(C0745.vM().isUsaCorPlacar());
         this.setCorPlacar(C0745.vM().getCorPlacar());
         this.setUsaSons(C0745.vM().isUsaSons());
         this.setUsaCoresLista(C0745.vM().isUsaCoresLista());
         this.setCoresLista(C0745.vM().getCoresLista());
         this.setNegritoCasa(C0745.vM().isNegritoCasa());
         this.setVerEstaduaisAgrupados(C0745.vM().isVerEstaduaisAgrupados());
         this.setUsarGruposReaisCopa(C0745.vM().isUsarGruposReaisCopa());
         this.setJogaRegionaisTodos(C0745.vM().getJogaRegionaisTodos());
         this.setVerLeiloes(C0745.vM().isVerLeiloes());
      }

      this.G();
   }

   public void G() {
      this.bx = new ArrayList();
      this.by = new ArrayList();
      this.bz = new ArrayList();
      this.bA = new ArrayList();
      this.bB = new ArrayList();
   }

   public int H() {
      return this.ae;
   }

   public void k(int i) {
      this.ae = i;
   }

   public void I() {
      this.ae++;
   }

   public int J() {
      return this.af;
   }

   public void l(int i) {
      this.af = i;
   }

   public ArrayList L() {
      return this.al;
   }

   public void a(ArrayList arrayList) {
      this.al = arrayList;
   }

   public ArrayList M() {
      return this.am;
   }

   public void b(ArrayList arrayList) {
      this.am = arrayList;
   }

   public ArrayList N() {
      return this.ao;
   }

   public void a(C0692 c0692) {
      this.ao.add(c0692);
   }

   public ArrayList O() {
      return this.ag;
   }

   public void c(ArrayList arrayList) {
      this.ag = arrayList;
   }

   public ArrayList P() {
      return this.aj;
   }

   public void d(ArrayList arrayList) {
      this.aj = arrayList;
   }

   public void c(Club club) {
      this.aj.add(club);
   }

   public void b(Player player) {
      this.ag.add(player);
   }

   public void c(Player player) {
      this.ah.add(player);
   }

   public void a(C0693 c0693) {
      this.as.add(c0693);
   }

   public ArrayList Q() {
      return this.ah;
   }

   public void e(ArrayList arrayList) {
      this.ah = arrayList;
   }

   public void a(Coach coach) {
      this.al.add(coach);
   }

   public void b(Coach coach) {
      this.am.add(coach);
   }

   public ArrayList R() {
      return this.as;
   }

   public ArrayList S() {
      return ((C0693)this.as.get(this.af)).h();
   }

   public ArrayList m(int i) {
      return i < this.as.size() ? ((C0693)this.as.get(i)).h() : null;
   }

   public String T() {
      return ((C0693)this.as.get(this.af)).f();
   }

   private static boolean U() {
      return true;
   }

   public void V() {
      if (!this.bk) {
         this.ap();
      } else {
         this.aa();
      }
   }

   public int iU() {
      return C0745.SR.op() - 1;
   }

   private void W() {
      ArrayList var1 = new ArrayList();

      for (int var2 = 0; var2 < C0745.SR.bo().size(); var2++) {
         if (((C0709)C0745.SR.bo().get(var2)).getY() + 10 < this.iU()) {
            var1.add((C0709)C0745.SR.bo().get(var2));
         }
      }

      for (int var3 = 0; var3 < var1.size(); var3++) {
         C0745.SR.bo().remove(var1.get(var3));
      }
   }

   public boolean X() {
      return C0745.SR.isJogaRegionais();
   }

   private void Y() {
      C0692 var1 = null;

      for (int var2 = 0; var2 < C0745.SR.N().size(); var2++) {
         if (((C0692)C0745.SR.N().get(var2)).jc() == 29) {
            var1 = (C0692)C0745.SR.N().get(var2);
         }
      }

      if (var1 != null) {
         var1.y(true);
      }
   }

   private void Z() {
      if (this.ar != null && this.ar.size() != 0) {
         for (int var6 = 0; var6 < C0745.SR.bW().size(); var6++) {
            ((C0931)C0745.SR.bW().get(var6)).Ab();
            ((C0931)C0745.SR.bW().get(var6)).mw();
         }
      } else {
         for (int var1 = 0; var1 < C0745.SR.N().size(); var1++) {
            Object var2 = null;
            Object var3 = null;
            var2 = ((C0692)C0745.SR.N().get(var1)).jq().cv(C0745.SR.H() - 1);
            Club[] var4 = ((C0692)C0745.SR.N().get(var1)).bk(C0745.SR.H() - 1);
            if (var4 != null) {
               if (var2 != var4[0]) {
                  var3 = var4[0];
               } else {
                  var3 = var4[1];
               }
            }

            C0931 var5 = new C0931(((C0692)C0745.SR.N().get(var1)).jc());
            C0745.SR.ar.add(var5);
            var5.Ab();
         }
      }
   }

   private void aa() {
      for (int var1 = 0; var1 < C0745.SR.aN().size(); var1++) {
         ((Club)C0745.SR.aN().get(var1)).kI();
      }

      if (this.jogaIntClubes) {
         this.ai();
      }

      if (this.jogaSelecoesAll) {
         this.jw();
      }

      this.ah();
      this.ag();
      this.iZ();
      if (this.jogaIntClubes) {
         this.ae();
      }

      this.Y();
      this.Z();
      this.am();
      this.ak();
      if (C0745.SR.jogaRegionais) {
         this.aj();
      }

      if (C0745.SR.bk() || C0745.SR.X()) {
         int var3 = C0693.e(1);
         if (var3 > 0) {
            C0693.b(var3, 4, 1);
         }
      }

      C0693.q();
      this.af();
      if (C0745.SR.H() >= 5) {
         this.W();
      }

      C0745.afQ.zi();
      C0745.afQ.zj();
      C0700.jO();
      C0700.jQ();
      if (C0745.SR.isJogaSelecoesAll()) {
         this.ac();
      }

      C0745.SR.yn().xZ();
      this.yn().clear();
      boolean var4 = false;
      if (this.bs && !C0745.SR.bD()) {
         for (int var2 = 0; var2 < C0745.SR.M().size(); var2++) {
            if (((Coach)C0745.SR.M().get(var2)).fg() != null) {
               var4 = true;
               break;
            }
         }

         if (var4) {
            C0151.pl();
         }
      }

      if (C0745.SR.yL()) {
         this.iY();
      }

      this.bk = false;
      if (var4) {
         C0151.AC();
      } else {
         C0745.SR.az();
         this.V();
      }
   }

   public void iX() {
      int var1 = -1;
      var1 = JOptionPane.showConfirmDialog(null, "Deseja criar um torneio Amistoso de início de temporada? ", "Torneio Amistoso", 0);
      if (var1 == 0) {
         C0685.aY(10);
      } else {
         this.bk = false;
         this.V();
      }
   }

   private void iY() {
      String var1 = "Torneio Amistoso";
      String var2 = "Juazeiro";
      ArrayList var3 = new ArrayList();

      for (int var4 = 0; var4 <= 7; var4++) {
         var3.add((Club)C0745.SR.P().get(var4));
      }

      if (C0745.SR.yn() != null) {
         C0745.SR.yn().a(var3, 0, var1, var2, false);
      }
   }

   public void a(String string) {
      Club var2 = null;
      Object var3 = null;
      int var4 = 0;

      for (int var5 = 0; var5 < C0745.SR.O().size(); var5++) {
         var4 = 0;
         var3 = (Player)C0745.SR.O().get(var5);

         for (int var6 = 0; var6 < C0745.SR.P().size(); var6++) {
            var2 = (Club)C0745.SR.P().get(var6);

            for (int var7 = 0; var7 < var2.kc().size(); var7++) {
               if (((Player)var2.kc().get(var7)).equals(var3)) {
                  var4++;
               }
            }
         }
      }
   }

   public void b(String string) {
      Object var2 = null;
      Object var3 = null;
      boolean var4 = false;

      for (int var5 = 0; var5 < C0745.SR.O().size(); var5++) {
         var4 = false;
         var3 = (Player)C0745.SR.O().get(var5);

         for (int var6 = 0; var6 < C0745.SR.P().size(); var6++) {
            Collections.frequency(((Club)C0745.SR.P().get(var6)).kc(), var3);
         }
      }
   }

   public void ab() {
      C0950 var1 = new C0950();
      this.a(var1);
      C0948 var2 = new C0948();
      this.a(var2);
      C0945 var3 = new C0945();
      this.a(var3);
      C0947 var4 = new C0947();
      this.a(var4);
      C0946 var5 = new C0946();
      this.a(var5);
      C0949 var6 = new C0949();
      this.a(var6);
      C0943 var7 = new C0943();
      this.a(var7);
      C0944 var8 = new C0944();
      this.a(var8);
      C0941 var9 = new C0941();
      this.a(var9);
      C0939 var10 = new C0939();
      this.a(var10);
      C0940 var11 = new C0940();
      this.a(var11);
      C0952 var12 = new C0952();
      this.a(var12);
      C0953 var13 = new C0953();
      this.a(var13);
      C0933 var14 = new C0933();
      this.a(var14);
      C0935 var15 = new C0935();
      this.a(var15);
      C0936 var16 = new C0936();
      this.a(var16);
      var1.setNome(C0679.getString("eliAS"));
      var2.setNome(C0679.getString("eliEU"));
      var3.setNome(C0679.getString("eliAfr"));
      var5.setNome(C0679.getString("eliAsia"));
      var4.setNome(C0679.getString("eliConcacaf"));
      var6.setNome(C0679.getString("eliOce"));
      var12.setNome(C0679.getString("cro"));
      var11.setNome(C0679.getString("cam"));
      var13.setNome(C0679.getString("ncm"));
      var10.setNome(C0679.getString("nCAfr"));
      var9.setNome(C0679.getString("nCAsi"));
      var8.setNome(C0679.getString("nCOuro"));
      var7.setNome(C0679.getString("nCopaOFC"));
      var14.setNome("Finalíssima");
      var15.setNome("Liga Nações Europa");
      var16.setNome("Liga Nações Concacaf");
   }

   public void ac() {
      C0745.SR.ad();
      if (this.aK != null) {
         this.aK.xZ();
         this.aK.mw();
      }

      if (this.aS != null) {
         this.aS.xZ();
         this.aS.mw();
      }

      if (this.aR != null) {
         this.aR.xZ();
         this.aR.mw();
      }

      if (this.aT != null) {
         this.aT.xZ();
         this.aT.mw();
      }

      if (this.aU != null) {
         this.aU.xZ();
         this.aU.mw();
      }

      if (this.aV != null) {
         this.aV.xZ();
         this.aV.mw();
      }

      if (this.aW != null) {
         this.aW.xZ();
         this.aW.mw();
      }

      if (this.IX != null) {
         this.IX.mw();
      }

      if (this.GC != null) {
         this.GC.mw();
         this.GC.xZ();
      }

      if (this.GI != null) {
         this.GI.mw();
         this.GI.xZ();
      }

      if (this.aL != null) {
         this.aL.xZ();
      }

      if (this.aM != null) {
         this.aM.xZ();
      }

      if (this.aN != null) {
         this.aN.xZ();
      }

      if (this.aQ != null) {
         this.aQ.xZ();
      }

      if (this.aO != null) {
         this.aO.xZ();
      }

      if (this.aP != null) {
         this.aP.xZ();
      }

      if (this.aL != null) {
         this.aL.mw();
      }

      if (this.aM != null) {
         this.aM.mw();
      }

      if (this.aN != null) {
         this.aN.mw();
      }

      if (this.aQ != null) {
         this.aQ.mw();
      }

      if (this.aO != null) {
         this.aO.mw();
      }

      if (this.aP != null) {
         this.aP.mw();
      }

      if (this.bl == this.ae) {
         if (this.aK != null) {
            this.aK.ya();
         } else {
            C0953 var1 = new C0953();
            this.a(var1);
            this.aK.ya();
         }

         this.bl += 4;
      }

      if (this.bm - 1 == this.ae) {
         if (this.aM != null) {
            this.aM.fF(0);
         } else {
            C0948 var2 = new C0948();
            this.a(var2);
            var2.setNome(C0679.getString("eliEU"));
            this.aM.fF(0);
         }
      }

      if (this.bl - 1 == this.ae) {
         if (this.aL != null) {
            this.aL.ya();
         } else {
            C0950 var3 = new C0950();
            this.a(var3);
            var3.setNome(C0679.getString("eliAS"));
            this.aL.ya();
         }

         if (this.aM != null) {
            this.aM.fF(7);
         } else {
            C0948 var4 = new C0948();
            this.a(var4);
            var4.setNome(C0679.getString("eliEU"));
            this.aM.fF(7);
         }

         if (this.aN != null) {
            this.aN.Bj();
         } else {
            C0945 var5 = new C0945();
            this.a(var5);
            var5.setNome(C0679.getString("eliAfr"));
            this.aN.Bj();
         }

         if (this.aQ != null) {
            this.aQ.Bj();
         } else {
            C0947 var6 = new C0947();
            this.a(var6);
            var6.setNome(C0679.getString("eliConcacaf"));
            this.aQ.Bj();
         }

         if (this.aO != null) {
            this.aO.Bj();
         } else {
            C0946 var7 = new C0946();
            this.a(var7);
            var7.setNome(C0679.getString("eliAsia"));
            this.aO.Bj();
         }

         if (this.aP != null) {
            this.aP.Bj();
         } else {
            C0949 var8 = new C0949();
            this.a(var8);
            var8.setNome(C0679.getString("eliOce"));
            this.aP.Bj();
         }
      }

      if (this.bm == this.ae) {
         if (this.aS != null) {
            this.aS.ya();
         } else {
            C0952 var9 = new C0952();
            this.a(var9);
            var9.setNome(C0679.getString("cro"));
            this.aS.ya();
         }

         this.bm += 4;
      } else if (this.aS != null) {
         this.aS.p((C0955)null);
      }

      if (this.bn == this.ae) {
         if (this.aR != null) {
            this.aR.ya();
         } else {
            C0940 var10 = new C0940();
            this.a(var10);
            var10.setNome(C0679.getString("cam"));
            this.aR.ya();
         }

         if (this.ae == 1) {
            this.bn += 4;
         } else {
            this.bn += 4;
         }
      } else if (this.aR != null) {
         this.aR.p((C0955)null);
      }

      if (this.bo == this.ae) {
         if (this.aT != null) {
            this.aT.ya();
         } else {
            this.a(new C0939());
            this.aT.ya();
         }

         this.bo += 4;
      } else if (this.aT != null) {
         this.aT.p((C0955)null);
      }

      if (this.bp == this.ae) {
         if (this.aU != null) {
            this.aU.ya();
         } else {
            this.a(new C0941());
            this.aU.ya();
         }

         this.bp += 4;
      } else if (this.aU != null) {
         this.aU.p((C0955)null);
      }

      if (this.br == this.ae) {
         if (this.aV != null) {
            this.aV.ya();
         } else {
            this.a(new C0944());
            this.aV.ya();
         }

         this.br += 2;
      } else if (this.aV != null) {
         this.aV.p((C0955)null);
      }

      if (this.bq == this.ae) {
         if (this.aW != null) {
            this.aW.ya();
         } else {
            this.a(new C0943());
            this.aW.ya();
         }

         this.bq += 4;
      } else if (this.aW != null) {
         this.aW.p((C0955)null);
      }

      if (this.IZ == this.ae) {
         if (this.IX != null) {
            this.IX.Bs();
         }

         this.IZ += 4;
      }

      if (this.Ja == this.ae) {
         if (this.GC != null) {
            this.GC.Ar();
         }

         this.Ja += 2;
      }

      if (this.Jb == this.ae) {
         if (this.GI != null) {
            this.GI.Ar();
         }

         this.Jb += 2;
      }
   }

   public void ad() {
      for (int var1 = 0; var1 < this.ap.size(); var1++) {
         ((C0692)this.ap.get(var1)).jm();
         ((C0692)this.ap.get(var1)).jj();
      }
   }

   public void ae() {
      for (int var1 = 0; var1 < C0745.SR.aG().size(); var1++) {
         ((C0692)C0745.SR.aG().get(var1)).jy();
      }

      this.aP().yq();
      this.aF().yq();
      this.aI().yq();
      this.aO().yq();
      this.aL().yq();
      this.aQ().yq();
      this.aH().yq();
      this.aK().yq();
      this.mj().yq();
      this.aH().mw();
      this.aF().mw();
      this.aI().mw();
      this.aO().mw();
      this.aL().mw();
      this.aQ().mw();
      this.aP().mw();
      this.aH().mw();
      this.aK().mw();
      this.mj().mw();
      this.aR().mw();
      if (C0745.SR.aW() != null) {
         this.aW().mw();
      }

      if (C0745.SR.aV() != null) {
         this.aV().mw();
      }

      if (this.aY() != null) {
         this.aY().mw();
      }

      if (this.ba() != null) {
         this.ba().mw();
      }

      if (this.aZ() != null) {
         this.aZ().mw();
      }

      if (this.aT != null) {
         this.aT.xZ();
         this.aT.mw();
      }

      if (this.aU != null) {
         this.aU.xZ();
         this.aU.mw();
      }

      if (this.aV != null) {
         this.aV.xZ();
         this.aV.mw();
      }

      if (this.aW != null) {
         this.aW.xZ();
         this.aW.mw();
      }

      int var3 = C0693.d(4);
      if (var3 > 0) {
         C0693.b(var3, 1, 0);
      }

      if (C0745.SR.aV() != null) {
         C0745.SR.aV().zR();
      } else {
         C0927 var2 = new C0927();
         C0745.SR.a(var2);
         var2.zR();
      }

      if (C0745.SR.aW() != null) {
         C0745.SR.aW().zR();
      } else {
         C0926 var4 = new C0926();
         C0745.SR.a(var4);
         var4.zR();
      }

      if (C0745.SR.aV() != null) {
         C0745.SR.aV().mw();
      }

      if (C0745.SR.aW() != null) {
         C0745.SR.aW().mw();
      }
   }

   public void c(String string) {
      Object var2 = null;
      Object var3 = null;
      boolean var4 = false;

      for (int var5 = 0; var5 < C0745.SR.Q().size(); var5++) {
         var4 = false;
         var3 = (Player)C0745.SR.Q().get(var5);

         for (int var6 = 0; var6 < C0745.SR.P().size(); var6++) {
            Collections.frequency(((Club)C0745.SR.P().get(var6)).ky(), var3);
         }
      }
   }

   private void af() {
      this.by.clear();
      this.bz.clear();
      this.bA.clear();
      this.bB.clear();

      for (int var1 = 0; var1 < this.ai.size(); var1++) {
         ((Player)this.ai.get(var1)).gc();
      }

      for (int var2 = 0; var2 < this.ag.size(); var2++) {
         ((Player)this.O().get(var2)).gd();
      }

      for (int var3 = 0; var3 < this.bz.size(); var3++) {
         this.ag.remove(this.bz.get(var3));
      }

      for (int var4 = 0; var4 < this.by.size(); var4++) {
         this.ag.add((Player)this.by.get(var4));
      }

      this.by.clear();
      this.bz.clear();

      for (int var5 = 0; var5 < this.ah.size(); var5++) {
         ((Player)this.Q().get(var5)).gf();
      }

      this.c(true);

      for (int var6 = 0; var6 < this.bB.size(); var6++) {
         this.ah.remove(this.bB.get(var6));
      }

      for (int var7 = 0; var7 < this.bA.size(); var7++) {
         if (!this.ah.contains(this.bA.get(var7))) {
            this.ah.add((Player)this.bA.get(var7));
         }
      }

      this.bA.clear();
      this.bB.clear();

      for (int var8 = 0; var8 < C0745.SR.O().size(); var8++) {
         ((Player)C0745.SR.O().get(var8)).fK();
         ((Player)C0745.SR.O().get(var8)).fJ();
      }
   }

   public void c(boolean bl) {
      int var2 = 0;
      if (!bl) {
         int[] var3 = new int[]{0, 1, 2, 3, 3, 4};
         int[] var4 = new int[]{0, 1, 2, 3, 4};

         for (int var5 = 0; var5 < this.aj.size(); var5++) {
            if (((Club)this.aj.get(var5)).kn() && ((Club)this.aj.get(var5)).getReputacao() >= 3) {
               if (((Club)this.aj.get(var5)).kx() < 12) {
                  for (int var10 = 0; var10 < var3.length; var10++) {
                     Player.a((Club)this.aj.get(var5), var3[var10], null, 0, null, bl);
                  }
               }
            } else if (((Club)this.aj.get(var5)).kx() < 5) {
               for (int var6 = 0; var6 < var4.length; var6++) {
                  Player.a((Club)this.aj.get(var5), var4[var6], null, 0, null, bl);
               }
            }
         }
      } else {
         for (int var8 = 0; var8 < this.aj.size(); var8++) {
            if (((Club)this.aj.get(var8)).kx() < 15) {
               if (((Club)this.aj.get(var8)).kn()) {
                  var2 = 2 + new Random().nextInt(3);
               } else {
                  var2 = new Random().nextInt(3);
                  if (((Club)this.aj.get(var8)).getNivel() >= 20) {
                     var2++;
                  }
               }

               if (var2 + ((Club)this.aj.get(var8)).kx() >= 20) {
                  var2 = 20 - ((Club)this.aj.get(var8)).kx() - 2;
               }

               if (var2 > 0) {
                  for (int var9 = 0; var9 <= var2; var9++) {
                     Player.a((Club)this.aj.get(var8), -1, null, 0, null, bl);
                  }
               }
            }
         }
      }
   }

   private void iZ() {
      for (int var1 = 0; var1 < this.al.size(); var1++) {
         ((Coach)this.al.get(var1)).kk();
      }
   }

   private void ag() {
      for (int var1 = 0; var1 < this.aj.size(); var1++) {
         ((Club)this.aj.get(var1)).km().clear();
         ((Club)this.aj.get(var1)).kk();
         if (!((Club)this.aj.get(var1)).jZ()) {
            ((Club)this.aj.get(var1)).kp();
            ((Club)this.aj.get(var1)).kA();
            ((Club)this.aj.get(var1)).kz();
         }

         if (((Club)this.aj.get(var1)).ka() != null) {
            ((Club)this.aj.get(var1)).ka().cj(50);
            ((Club)this.aj.get(var1)).ka().ck(50);
         }
      }

      for (int var2 = 0; var2 < C0745.SR.aN().size(); var2++) {
         ((Club)C0745.SR.aN().get(var2)).kG();
      }
   }

   private void ah() {
      this.I();

      for (int var1 = 0; var1 < this.as.size(); var1++) {
         ((C0693)this.as.get(var1)).clear();
      }

      this.as.clear();
      this.r(C0745.SR.op() + (this.ae - 1));
      C0693.g();
   }

   private void ai() {
      if (C0745.SR.aK() != null && C0745.SR.aI() != null) {
         Club var1 = C0745.SR.aK().cS();
         C0745.SR.aI().T(var1);
         C0745.SR.aK().N(var1);
      }

      if (C0745.SR.aF() != null && C0745.SR.aH() != null) {
         Club var2 = C0745.SR.aH().cS();
         C0745.SR.aF().O(var2);
         C0745.SR.aH().N(var2);
      }

      if (C0745.SR.aK() != null && C0745.SR.mj() != null) {
         Club var3 = C0745.SR.mj().cS();
         C0745.SR.aK().ab(var3);
      }
   }

   private void jw() {
      if (this.IX != null && this.aS != null) {
         Club var1 = this.aS.cS();
         if (var1 != null) {
            this.IX.Z(var1);
         }
      }

      if (this.IX != null && this.aR != null) {
         Club var2 = this.aR.cS();
         if (var2 != null) {
            this.IX.aa(var2);
         }
      }
   }

   private void aj() {
      new ArrayList();
      ArrayList var1 = C0734.di();

      for (int var2 = 0; var2 < this.aZ.length; var2++) {
         if (this.aZ[var2] != null) {
            this.aZ[var2].am(var1);
         }
      }
   }

   private void ak() {
      for (int var1 = 0; var1 < C0745.SR.aE().size(); var1++) {
         ((C0741)C0745.SR.aE().get(var1)).ee();
      }

      for (int var3 = 0; var3 < C0745.SR.aE().size(); var3++) {
         for (int var2 = 0; var2 < ((C0741)C0745.SR.aE().get(var3)).eb().size(); var2++) {
            ((C0951)((C0741)C0745.SR.aE().get(var3)).eb().get(var2)).yi().aN(false);
         }
      }
   }

   public void al() {
      for (int var1 = 0; var1 < C0745.SR.N().size(); var1++) {
         for (int var2 = 0; var2 < ((C0692)C0745.SR.N().get(var1)).eb().size(); var2++) {
            ((C0924)((C0692)C0745.SR.N().get(var1)).eb().get(var2)).fi(var2);
         }
      }

      for (int var3 = 0; var3 < C0745.SR.O().size(); var3++) {
         ((Player)C0745.SR.O().get(var3)).ay(0);
      }

      for (int var4 = 0; var4 < C0745.SR.N().size(); var4++) {
         for (int var5 = 0; var5 < ((C0924)((C0692)C0745.SR.N().get(var4)).eb().get(0)).zM().size(); var5++) {
            ((Player)((C0924)((C0692)C0745.SR.N().get(var4)).eb().get(0)).zM().get(var5)).fd();
         }
      }
   }

   private void am() {
      for (int var1 = 0; var1 < C0745.SR.N().size(); var1++) {
         ((C0692)C0745.SR.N().get(var1)).ee();
         ((C0692)C0745.SR.N().get(var1)).jq().mw();
      }

      for (int var6 = 0; var6 < C0745.SR.N().size(); var6++) {
         for (int var2 = 0; var2 < ((C0692)C0745.SR.N().get(var6)).eb().size(); var2++) {
            ((C0924)((C0692)C0745.SR.N().get(var6)).eb().get(var2)).yi().aN(false);
            ((C0924)((C0692)C0745.SR.N().get(var6)).eb().get(var2)).aT(false);
         }
      }

      for (int var7 = 0; var7 < C0745.SR.N().size(); var7++) {
         boolean var8 = true;
         if (((C0692)C0745.SR.N().get(var7)).jc() == 29 && C0745.SR.bk()) {
            var8 = false;
         } else if (((C0692)C0745.SR.N().get(var7)).jc() == 29 && C0745.SR.X()) {
            var8 = false;
         }

         if (!var8) {
            if (((C0692)C0745.SR.N().get(var7)).jc() == 29) {
               ((C0692)C0745.SR.N().get(var7)).a((C0942)null);
            }
         } else {
            ArrayList var3 = new ArrayList();

            for (int var4 = 0; var4 < ((C0692)C0745.SR.N().get(var7)).eb().size(); var4++) {
               for (int var5 = 0; var5 < ((C0924)((C0692)C0745.SR.N().get(var7)).eb().get(var4)).yi().yK().size(); var5++) {
                  var3.add((Club)((C0924)((C0692)C0745.SR.N().get(var7)).eb().get(var4)).yi().yK().get(var5));
               }
            }

            ArrayList var9 = new ArrayList();
            var9.addAll(((C0692)C0745.SR.N().get(var7)).jg());
            Collections.sort(var9, C1007.cN);

            for (int var10 = 0; var10 < var9.size(); var10++) {
               if (!var3.contains(var9.get(var10))) {
                  var3.add((Club)var9.get(var10));
               }
            }

            ((C0692)C0745.SR.N().get(var7)).a(var3, false, "atualizaLigasNacionais");
         }
      }
   }

   public static void an() {
      new ArrayList();
      ArrayList var0 = ((C0693)C0745.SR.R().get(C0745.SR.af)).j();

      for (int var1 = 0; var1 < var0.size(); var1++) {
         if (var0.get(var1) instanceof C0955 && ((C0678)var0.get(var1)).b() == 1 && ((C0955)var0.get(var1)).vl() != null) {
            ((C0955)var0.get(var1)).vl().a((C0955)var0.get(var1));
         }
      }
   }

   public int ao() {
      return ((C0693)C0745.SR.R().get(C0745.SR.J())).t().size() > 0 ? ((C0713)((C0693)C0745.SR.R().get(C0745.SR.J())).t().get(0)).b() : -1;
   }

   public void ap() {
      this.bH = false;
      if (C0745.SR.bD() && C0745.SR.H() == 10) {
         C0745.SR.h(false);
      }

      if (ay()) {
         if (this.bs) {
            C0685.aY(2);
            if (!this.bL) {
               if (C0745.SR.autoSalvar == 1) {
                  if (this.bJ >= 3) {
                     try {
                        new java.io.ByteArrayInputStream(new byte[0]).close();
                        C0685.iF().ru();
                     } catch (IOException var4) {
                        var4.printStackTrace();
                     }

                     this.bJ = 0;
                  } else {
                     this.bJ++;
                  }
               } else if (C0745.SR.autoSalvar == 2) {
                  try {
                     new java.io.ByteArrayInputStream(new byte[0]).close();
                        C0685.iF().ru();
                  } catch (IOException var3) {
                     var3.printStackTrace();
                  }
               } else if (C0745.SR.autoSalvar == 3 && this.bF()) {
                  this.bM = false;

                  try {
                     new java.io.ByteArrayInputStream(new byte[0]).close();
                        C0685.iF().ru();
                  } catch (IOException var2) {
                     var2.printStackTrace();
                  }
               }
            }

            if (!this.bL && !C0685.iF().rB()) {
               if (new Random().nextInt(100) == 1) {
                  C0685.iF().ac(false);
               } else if (new Random().nextInt(2) == 1) {
                  C0685.iF().ac(true);
               }
            }
         }
      } else {
         if (C0745.SR.isIgnoraLigas() && this.ao() == 1 && !C0737.dG() && !this.bI) {
            this.bH = true;
         }

         if (this.bI) {
            this.bI = false;
         }

         if (C0745.SR.isIgnoraEstadual() && this.ao() == 3) {
            if (!C0737.J(3)) {
               this.bH = true;
            }
         } else if (C0745.SR.isIgnoraEstadual() && this.ao() == 10 && !C0737.J(10)) {
            this.bH = true;
         }

         if (!this.bH && this.ao() == 10 && this.aq() && !C0737.dH()) {
            this.bH = true;
         } else if (!this.bH && this.ao() == 10 && !this.ar() && C0745.vM().getRegionaisSemHumanos() == 0) {
            this.bH = true;
         }

         Club.kh();
         C0675.he();
         if (this.bs && !this.bH && C0737.dw()) {
            C0737.dx();
         } else {
            this.at();
         }
      }

      if (this.bL) {
         this.bL = false;
      }
   }

   private boolean aq() {
      int var1 = 0;

      for (int var2 = 0; var2 < C0745.SR.aZ.length; var2++) {
         if (C0745.SR.aZ[var2] != null) {
            var1++;
         }
      }

      return var1 < 4 && var1 > 0;
   }

   private boolean ar() {
      for (int var1 = 0; var1 < C0745.SR.M().size(); var1++) {
         if (C0745.SR.M().get(var1) != null) {
            Club var2 = ((Coach)C0745.SR.M().get(var1)).fg();
            if (var2 != null) {
               for (int var3 = 0; var3 < C0745.SR.aZ.length; var3++) {
                  if (C0745.SR.aZ[var3] != null
                     && C0745.SR.aZ[var3].yd() != null
                     && C0745.SR.aZ[var3].yd().yK() != null
                     && C0745.SR.aZ[var3].yd().yK().contains(var2)) {
                     return true;
                  }
               }
            }
         }
      }

      return false;
   }

   public void as() {
      new ArrayList();
      ArrayList var1 = ((C0693)C0745.SR.R().get(C0745.SR.J())).h();
      int var2 = ((C0693)C0745.SR.R().get(C0745.SR.J())).b();

      for (int var3 = 0; var3 < var1.size(); var3++) {
         ((C0675)var1.get(var3)).a(true);
         ((C0675)var1.get(var3)).hg();
         ((C0675)var1.get(var3)).hi();
         ((C0675)var1.get(var3)).hf();
         ((C0675)var1.get(var3)).hh();
         ((C0675)var1.get(var3)).hc().I(false);
         ((C0675)var1.get(var3)).hd().I(false);
         ((C0675)var1.get(var3)).a(null);
      }

      for (int var6 = 0; var6 < C0745.SR.O().size(); var6++) {
         ((Player)C0745.SR.O().get(var6)).fr();
      }

      for (int var7 = 0; var7 < C0745.SR.Q().size(); var7++) {
         ((Player)C0745.SR.Q().get(var7)).ai(100);
      }

      if (var2 == 7 || var2 == 9) {
         for (int var8 = 0; var8 < C0745.SR.aG().size(); var8++) {
            Club var4 = ((C0692)C0745.SR.aG().get(var8)).jn();
            if (var4 != null) {
               for (int var5 = 0; var5 < var4.kc().size(); var5++) {
                  if (((Player)var4.kc().get(var5)).fg() == null) {
                     ((Player)var4.kc().get(var5)).fr();
                  }
               }
            }
         }
      }

      Collections.sort(C0745.SR.L(), bO);
   }

   public void at() {
      this.as();
      an();
      this.aM();
      this.aC();
      this.aB();
      this.q(0);
      if (this.bs && !this.bH) {
         C0685.aY(5);
      } else {
         this.ax();
      }
   }

   private void au() {
      ArrayList var1 = new ArrayList();
      if (this.av != null && C0745.SR.av.size() > 0) {
         for (int var2 = 0; var2 < C0745.SR.av.size(); var2++) {
            if (((C0825)this.av.get(var2)).tM()) {
               boolean var3 = false;
               Coach var4 = null;
               if (((C0825)this.av.get(var2)).x().fg() != null && ((C0825)this.av.get(var2)).x().fg().jZ()) {
                  var3 = true;
                  var4 = ((C0825)this.av.get(var2)).x().fg().ka();
               }

               if (((C0825)this.av.get(var2)).tN()) {
                  var1.add((C0825)C0745.SR.av.get(var2));
                  if (((C0825)this.av.get(var2)).tP() != null && ((C0825)this.av.get(var2)).tP().jZ()) {
                     new C0799(((C0825)this.av.get(var2)).tP().ka(), 29, 83, "", ((C0825)this.av.get(var2)).x().getNome());
                  } else if (var3) {
                     new C0799(var4, 29, 84, "", ((C0825)this.av.get(var2)).x().getNome());
                  }
               } else if (((C0825)this.av.get(var2)).tP() != null && ((C0825)this.av.get(var2)).tP().jZ() && !((C0825)this.av.get(var2)).tQ()) {
                  new C0799(((C0825)this.av.get(var2)).tP().ka(), 28, 82, "", ((C0825)this.av.get(var2)).x().getNome());
                  ((C0825)this.av.get(var2)).au(true);
               }
            }
         }
      }

      if (var1.size() > 0) {
         for (int var5 = 0; var5 < var1.size(); var5++) {
            C0745.SR.av.remove(var1.get(var5));
         }
      }
   }

   public void d(Player player) {
      int var2 = -1;
      if (this.av != null && C0745.SR.av.size() > 0) {
         for (int var3 = 0; var3 < C0745.SR.av.size(); var3++) {
            if (((C0825)C0745.SR.av.get(var3)).x() == player) {
               var2 = var3;
               break;
            }
         }
      }

      if (var2 >= 0) {
         C0745.SR.av.remove(var2);
      }
   }

   private void av() {
      for (int var1 = 0; var1 < C0745.SR.aN().size(); var1++) {
         for (int var2 = 0; var2 < ((Club)C0745.SR.aN().get(var1)).kc().size(); var2++) {
            if (((Player)((Club)C0745.SR.aN().get(var1)).kc().get(var2)).ft()) {
               new C0794((Player)((Club)C0745.SR.aN().get(var1)).kc().get(var2), ((Player)((Club)C0745.SR.aN().get(var1)).kc().get(var2)).fl(), true, true);
            }
         }
      }
   }

   private void aw() {
      ArrayList var1 = new ArrayList();
      if (this.aw != null && C0745.SR.aw.size() > 0) {
         for (int var2 = 0; var2 < C0745.SR.aw.size(); var2++) {
            if (((C0668)C0745.SR.aw.get(var2)).ew().before(C0745.SR.bb())) {
               ((C0668)C0745.SR.aw.get(var2)).ex();
               var1.add((C0668)C0745.SR.aw.get(var2));
            }
         }
      }

      if (var1.size() > 0) {
         for (int var3 = 0; var3 < var1.size(); var3++) {
            C0745.SR.aw.remove(var1.get(var3));
         }
      }
   }

   public void ax() {
      int var1 = ((C0693)C0745.SR.R().get(C0745.SR.J())).i();
      if (var1 == 1 || var1 == 3) {
         this.n(var1);
      }

      int var2 = this.az();
      if (this.av != null && C0745.SR.av.size() > 0) {
         this.au();
      }

      if (new Random().nextInt(100) > 50) {
         this.av();
      }

      if (this.aw != null && C0745.SR.aw.size() > 0) {
         this.aw();
      }

      this.q(1);
      if (var2 == 0) {
         this.bk = true;
      }

      if (this.bs) {
         if (this.bk) {
            C0745.afQ.zw();
            this.al();
            C0685.aY(6);
         } else {
            C0745.SR.ap();
         }
      } else if (this.bk) {
         this.al();
      }
   }

   private static boolean ay() {
      new ArrayList();
      ArrayList var0 = ((C0693)C0745.SR.R().get(C0745.SR.af)).h();

      for (int var1 = 0; var1 < var0.size(); var1++) {
         if (((C0675)var0.get(var1)).hc().jZ() && !((C0675)var0.get(var1)).hc().kf()) {
            a(((C0675)var0.get(var1)).hc(), (C0675)var0.get(var1));
            return true;
         }

         if (((C0675)var0.get(var1)).hd().jZ() && !((C0675)var0.get(var1)).hd().kf()) {
            a(((C0675)var0.get(var1)).hd(), (C0675)var0.get(var1));
            return true;
         }
      }

      return false;
   }

   public static Club B() {
      return X;
   }

   public static void a(Club club, C0675 c0675) {
      X = club;
      bv = c0675;
   }

   public int az() {
      for (int var1 = C0745.SR.af; var1 < C0745.SR.R().size(); var1++) {
         if (!((C0693)C0745.SR.R().get(var1)).e() && ((C0693)C0745.SR.R().get(var1)).b() > 0 && ((C0693)C0745.SR.R().get(var1)).h().size() > 0) {
            C0745.SR.l(var1);
            return var1;
         }
      }

      return 0;
   }

   private static void aA() {
   }

   private void aB() {
      new ArrayList();
      ArrayList var1 = ((C0693)this.as.get(this.af)).t();

      for (int var2 = 0; var2 < var1.size(); var2++) {
         ((C0713)var1.get(var2)).mx();
      }
   }

   private void aC() {
      new ArrayList();
      ArrayList var1 = ((C0693)this.as.get(this.af)).j();

      for (int var2 = 0; var2 < var1.size(); var2++) {
         if (var1.get(var2) instanceof C0955) {
            ((C0955)var1.get(var2)).pO();
         } else if (var1.get(var2) instanceof C0962) {
            ((C0962)var1.get(var2)).pO();
         }
      }
   }

   private void n(int i) {
      for (int var3 = 0; var3 < this.am.size(); var3++) {
         if (((Coach)this.am.get(var3)).jZ() && ((Coach)this.am.get(var3)).fg() == null) {
            ArrayList var2 = C0745.afQ.b((Coach)this.am.get(var3), false);
            if (!C0745.SR.bD() && var2 != null && var2.size() > 0) {
               C0685.a(var2, (Coach)this.am.get(var3), 0);
            }
         }
      }

      ArrayList var5 = new ArrayList();
      if (i == 1) {
         for (int var4 = 0; var4 < this.ao.size(); var4++) {
            ((C0692)this.ao.get(var4)).A(var5);
         }
      } else if (i == 3) {
         for (int var6 = 0; var6 < this.aq.size(); var6++) {
            ((C0741)this.aq.get(var6)).A(var5);
         }
      }

      if (var5.size() > 0 && C0745.SR.bs && !C0745.SR.bD() && C0745.SR.getVerMudancaTecnicos() == 1) {
         C0685.K(var5);
      }

      boolean var7 = false;
   }

   public boolean isJogaEstadual() {
      return this.jogaEstadual;
   }

   public void setJogaEstadual(boolean bl) {
      this.jogaEstadual = bl;
   }

   public ArrayList aE() {
      return this.aq;
   }

   public C0954 aF() {
      return this.aC;
   }

   public void a(C0954 c0954) {
      this.aC = c0954;
   }

   public C0692 o(int i) {
      for (int var2 = 0; var2 < this.ao.size(); var2++) {
         if (((C0692)this.ao.get(var2)).jc() == i) {
            return (C0692)this.ao.get(var2);
         }
      }

      return null;
   }

   public C0741 p(int i) {
      for (int var2 = 0; var2 < this.aq.size(); var2++) {
         if (((C0741)this.aq.get(var2)).getEstado() == i) {
            return (C0741)this.aq.get(var2);
         }
      }

      return null;
   }

   public ArrayList aG() {
      return this.ap;
   }

   public C0930 aH() {
      return this.aY;
   }

   public void a(C0930 c0930) {
      this.aY = c0930;
   }

   public boolean isJogaIntClubes() {
      return this.jogaIntClubes;
   }

   public void setJogaIntClubes(boolean bl) {
      this.jogaIntClubes = bl;
   }

   public C0958 aI() {
      return this.aB;
   }

   public void a(C0958 c0958) {
      this.aB = c0958;
   }

   public boolean aJ() {
      return this.bb;
   }

   public void d(boolean bl) {
      this.bb = bl;
   }

   public C0960 aK() {
      return this.aX;
   }

   public void a(C0960 c0960) {
      this.aX = c0960;
   }

   public C0957 aL() {
      return this.aE;
   }

   public void a(C0957 c0957) {
      this.aE = c0957;
   }

   public void aM() {
      for (int var1 = 0; var1 <= this.af; var1++) {
         ((C0693)this.as.get(var1)).a(true);
      }
   }

   public void q(int i) {
      if (i == 0) {
         for (int var2 = 0; var2 <= this.af; var2++) {
            if (((C0693)this.as.get(var2)).r().size() > 0) {
               ((C0693)this.as.get(var2)).f(0);
            }
         }
      } else {
         for (int var3 = 0; var3 <= this.af; var3++) {
            if (((C0693)this.as.get(var3)).s().size() > 0) {
               ((C0693)this.as.get(var3)).f(1);
            }
         }
      }
   }

   public ArrayList aN() {
      return this.ak;
   }

   public void d(Club club) {
      this.ak.add(club);
   }

   public C0956 aO() {
      return this.aD;
   }

   public void a(C0956 c0956) {
      this.aD = c0956;
   }

   public C0959 aP() {
      return this.aF;
   }

   public C0961 aQ() {
      return this.aG;
   }

   public void a(C0961 c0961) {
      this.aG = c0961;
   }

   public void a(C0959 c0959) {
      this.aF = c0959;
   }

   public C0923 aR() {
      if (this.aA == null && C0745.SR.isJogaIntClubes()) {
         this.aA = new C0923();
      }

      return this.aA;
   }

   public void a(C0923 c0923) {
      this.aA = c0923;
   }

   public void r(int i) {
      Calendar var2 = Calendar.getInstance();
      DateFormat var3 = DateFormat.getDateInstance();
      int var4 = 0;
      boolean var5 = false;
      int var6 = 0;
      var2.set(i, 0, 1, 0, 0, 0);

      while (var2.get(1) == i) {
         C0693 var7 = new C0693();
         var7.a(var2.get(1), var2.get(2), var2.get(5));
         C0745.SR.a(var7);
         if (var2.get(2) == 0 && var2.get(7) == 1) {
            if (++var4 == 1) {
               C0745.SR.l(var6);
            }
         }

         var6++;
         var2.add(5, 1);
      }

      this.aS();
   }

   private void aS() {
      ArrayList var1 = C0693.d();
      ArrayList var2 = C0693.c(1);
      ArrayList var3;
      if (this.salarioMensal) {
         var3 = var1;
      } else {
         var3 = var2;
      }

      for (int var4 = 0; var4 < var3.size(); var4++) {
         C0693.b((Integer)var3.get(var4), 2, 0);
      }

      for (int var6 = 0; var6 < var1.size(); var6++) {
         C0693.b((Integer)var1.get(var6), 5, 0);
      }

      var3 = var2;

      for (int var7 = 0; var7 < var3.size(); var7++) {
         C0693.b((Integer)var3.get(var7), 3, 0);
      }
   }

   public boolean isUsaGrupoPadraoEstadual() {
      return this.usaGrupoPadraoEstadual;
   }

   public void setUsaGrupoPadraoEstadual(boolean bl) {
      this.usaGrupoPadraoEstadual = bl;
   }

   public void aT() {
      if (C0745.SR.isHabilidadeIndividual() && C0745.SR.bO() == 3) {
         for (int var1 = 0; var1 < this.aj.size(); var1++) {
            ((Club)this.aj.get(var1)).lj();
         }
      }

      for (int var2 = 0; var2 < this.ag.size(); var2++) {
         ((Player)this.ag.get(var2)).fV();
      }

      for (int var3 = 0; var3 < this.ah.size(); var3++) {
         ((Player)this.ah.get(var3)).fY();
      }

      if (C0745.SR.isHabilidadeIndividual()) {
         if (C0745.SR.bO() == 3) {
            C0745.SR.A(0);
         } else {
            C0745.SR.bP();
         }
      }
   }

   public void aU() {
      for (int var1 = 0; var1 < this.O().size(); var1++) {
         ((Player)this.O().get(var1)).fI();
      }

      for (int var2 = 0; var2 < this.Q().size(); var2++) {
         ((Player)this.Q().get(var2)).fI();
      }

      for (int var3 = 0; var3 < this.aj.size(); var3++) {
         ((Club)this.aj.get(var3)).kp();
         ((Club)this.aj.get(var3)).kH();
      }
   }

   public C0927 aV() {
      return this.aI;
   }

   public void a(C0927 c0927) {
      this.aI = c0927;
   }

   public C0926 aW() {
      return this.aJ;
   }

   public void a(C0926 c0926) {
      this.aJ = c0926;
   }

   public boolean isSalarioMensal() {
      return this.salarioMensal;
   }

   public void setSalarioMensal(boolean bl) {
      this.salarioMensal = bl;
   }

   public ArrayList aX() {
      if (this.bx == null) {
         this.bx = new ArrayList();
      }

      return this.bx;
   }

   public C0953 aY() {
      return this.aK;
   }

   public void a(C0953 c0953) {
      this.aK = c0953;
   }

   public C0940 aZ() {
      return this.aR;
   }

   public void a(C0940 c0940) {
      this.aR = c0940;
   }

   public C0952 ba() {
      return this.aS;
   }

   public void a(C0952 c0952) {
      this.aS = c0952;
   }

   public C0692 s(int i) {
      for (int var2 = 0; var2 < this.ap.size(); var2++) {
         if (((C0692)this.ap.get(var2)).jc() == i) {
            return (C0692)this.ap.get(var2);
         }
      }

      return null;
   }

   public Calendar bb() {
      return ((C0693)this.as.get(this.af)).a();
   }

   public long bc() {
      return ((C0693)this.as.get(this.af)).a().getTime().getTime();
   }

   public ArrayList bd() {
      return this.ax;
   }

   public C0939 be() {
      return this.aT;
   }

   public void a(C0939 c0939) {
      this.aT = c0939;
   }

   public C0941 bf() {
      return this.aU;
   }

   public void a(C0941 c0941) {
      this.aU = c0941;
   }

   public C0944 bg() {
      return this.aV;
   }

   public void a(C0944 c0944) {
      this.aV = c0944;
   }

   public Coach bh() {
      for (int var1 = 0; var1 < this.al.size(); var1++) {
         if (((Coach)this.al.get(var1)).fg() == null && !((Coach)this.al.get(var1)).jZ()) {
            return (Coach)this.al.get(var1);
         }
      }

      return null;
   }

   public Coach a(Club club, int i) {
      ArrayList var3 = new ArrayList();
      int var4 = 0;
      int var5 = 0;

      for (int var6 = 0; var6 < this.al.size(); var6++) {
         if (i == -1) {
            var4 = 0;
            var5 = 5;
         } else if (i == 0) {
            var4 = club.getReputacao();
            var5 = club.getReputacao();
         } else if (i == 1) {
            var4 = club.getReputacao() - 1;
            var5 = club.getReputacao();
         } else if (i == 2) {
            var4 = club.getReputacao() - 2;
            var5 = club.getReputacao() + 1;
         }

         if (((Coach)this.al.get(var6)).fg() == null
            && !((Coach)this.al.get(var6)).jZ()
            && ((Coach)this.al.get(var6)).fg() != club
            && (((Coach)this.al.get(var6)).bz() == club.getPais() || ((Coach)this.al.get(var6)).lE() == club.getPais())
            && ((Coach)this.al.get(var6)).getReputacao() >= var4
            && ((Coach)this.al.get(var6)).getReputacao() <= var5) {
            var3.add((Coach)this.al.get(var6));
         }
      }

      if (var3.size() > 0) {
         int var7 = new Random().nextInt(100);
         if (var7 < 50) {
            Collections.sort(var3, bO);
         } else {
            Collections.shuffle(var3);
         }
      }

      return var3.size() > 0 ? (Coach)var3.get(0) : null;
   }

   public void a(Coach coach, Coach coach2) {
      Club var3 = coach.fg();
      Club var4 = coach2.fg();
      coach.i(coach2);
      coach2.i(coach);
      coach.E(var4);
      coach2.E(var3);
   }

   public void a(Club club, Coach coach, Coach coach2) {
      if (coach != null) {
         coach.i(coach2);
      }

      if (coach2 != null) {
         coach2.E(club);
      }
   }

   public boolean t(int i) {
      for (int var2 = 0; var2 < this.bF.size(); var2++) {
         if (((C0681)this.bF.get(var2)).getPais() == i && ((C0681)this.bF.get(var2)).iC()) {
            return true;
         }
      }

      return false;
   }

   public void bi() {
      for (int var1 = 0; var1 < this.bF.size(); var1++) {
         ((C0681)this.bF.get(var1)).v(true);
      }
   }

   public boolean bj() {
      for (int var1 = 0; var1 < this.bF.size(); var1++) {
         if (((C0681)this.bF.get(var1)).iC()) {
            return true;
         }
      }

      return false;
   }

   public boolean isJogaIntano1() {
      return this.jogaIntano1;
   }

   public void setJogaIntano1(boolean bl) {
      this.jogaIntano1 = bl;
   }

   public boolean isGruposIntPadrao() {
      return this.gruposIntPadrao;
   }

   public void setGruposIntPadrao(boolean bl) {
      this.gruposIntPadrao = bl;
   }

   public boolean[] getVerJint() {
      return this.verJint;
   }

   public void setVerJint(boolean[] bls) {
      this.verJint = bls;
   }

   public void setVerJint(boolean bl, int i) {
      this.verJint[i] = bl;
   }

   public boolean isJogaSelecoesAll() {
      return this.jogaSelecoesAll;
   }

   public void setJogaSelecoesAll(boolean bl) {
      this.jogaSelecoesAll = bl;
   }

   public boolean bk() {
      return this.bc;
   }

   public void e(boolean bl) {
      this.bc = bl;
   }

   public static C0675 bl() {
      return bv;
   }

   public boolean bm() {
      return this.bd;
   }

   public void f(boolean bl) {
      this.bd = bl;
   }

   public boolean isNegritoCasa() {
      return this.negritoCasa;
   }

   public void setNegritoCasa(boolean bl) {
      this.negritoCasa = bl;
   }

   public ArrayList bn() {
      return this.at;
   }

   public ArrayList bo() {
      return this.au;
   }

   public int getAutoSalvar() {
      return this.autoSalvar;
   }

   public void setAutoSalvar(int i) {
      this.autoSalvar = i;
   }

   public int getVelocidade() {
      return this.velocidade;
   }

   public void setVelocidade(int i) {
      this.velocidade = i;
   }

   public int getVelocidadeNH() {
      return this.velocidadeNH;
   }

   public void setVelocidadeNH(int i) {
      this.velocidadeNH = i;
   }

   public int getAvisoTerminoContrato() {
      return this.avisoTerminoContrato;
   }

   public void setAvisoTerminoContrato(int i) {
      this.avisoTerminoContrato = i;
   }

   public int getVerDecisaoPenNaoHumano() {
      return this.verDecisaoPenNaoHumano;
   }

   public void setVerDecisaoPenNaoHumano(int i) {
      this.verDecisaoPenNaoHumano = i;
   }

   public int getVerMudancaTecnicos() {
      return this.verMudancaTecnicos;
   }

   public void setVerMudancaTecnicos(int i) {
      this.verMudancaTecnicos = i;
   }

   public int getVerJanelaSubs() {
      return this.verJanelaSubs;
   }

   public void setVerJanelaSubs(int i) {
      this.verJanelaSubs = i;
   }

   public boolean isAutoRenovaContrato() {
      return this.autoRenovaContrato;
   }

   public void setAutoRenovaContrato(boolean bl) {
      this.autoRenovaContrato = bl;
   }

   public boolean isUsaCorPlacar() {
      return this.usaCorPlacar;
   }

   public void setUsaCorPlacar(boolean bl) {
      this.usaCorPlacar = bl;
   }

   public int getCorPlacar() {
      return this.corPlacar;
   }

   public void setCorPlacar(int i) {
      this.corPlacar = i;
   }

   public boolean isUsaSons() {
      return this.usaSons;
   }

   public void setUsaSons(boolean bl) {
      this.usaSons = bl;
   }

   public boolean isUsaCoresLista() {
      return this.usaCoresLista;
   }

   public void setUsaCoresLista(boolean bl) {
      this.usaCoresLista = bl;
   }

   public boolean isVerEstaduaisAgrupados() {
      return this.verEstaduaisAgrupados;
   }

   public void setVerEstaduaisAgrupados(boolean bl) {
      this.verEstaduaisAgrupados = bl;
   }

   public String[] bp() {
      return this.bP;
   }

   public void bq() {
      for (int var1 = 0; var1 < this.bP.length; var1++) {
         this.bP[var1] = String.format("#%02x%02x%02x", this.coresLista[0].getRed(), this.coresLista[var1].getGreen(), this.coresLista[var1].getBlue());
      }
   }

   public Color[] getCoresLista() {
      if (this.coresLista != null) {
         return this.coresLista;
      }

      this.coresLista = new Color[5];

      for (int var1 = 0; var1 < this.coresLista.length; var1++) {
         this.coresLista[var1] = new Color(
            Integer.valueOf(this.bP[var1].substring(1, 3), 16),
            Integer.valueOf(this.bP[var1].substring(3, 5), 16),
            Integer.valueOf(this.bP[var1].substring(5, 7), 16)
         );
      }

      return this.coresLista;
   }

   public void setCoresLista(Color[] colors) {
      this.coresLista = colors;
      this.bq();
   }

   public String br() {
      return this.bj;
   }

   public void d(String string) {
      this.bj = string;
   }

   public SavedGameInfo bs() {
      if (this.ad == null) {
         this.ad = new SavedGameInfo();
      }

      return this.ad;
   }

   public void a(SavedGameInfo savedGameInfo) {
      this.ad = savedGameInfo;
   }

   public ArrayList bt() {
      return this.av;
   }

   public ArrayList bu() {
      if (this.an == null) {
         this.an = new ArrayList();
      }

      return this.an;
   }

   public C0937 bv() {
      return this.aH;
   }

   public ArrayList bw() {
      if (this.aw == null) {
         this.aw = new ArrayList();
      }

      return this.aw;
   }

   public ArrayList bx() {
      return this.ay;
   }

   public int by() {
      return this.be;
   }

   public void u(int i) {
      this.be = i;
   }

   public int bz() {
      return this.bf;
   }

   public int bA() {
      byte var1 = -1;
      if (C0745.SR.M().size() > 0) {
         Coach var2 = (Coach)C0745.SR.M().get(0);
         return var2.fg() != null ? var2.fg().getPais() : var2.bz();
      } else {
         return var1;
      }
   }

   public void v(int i) {
      this.bf = i;
   }

   public void setIgnoraLigas(boolean bl) {
      this.ignoraLigas = bl;
   }

   public boolean isIgnoraLigas() {
      return this.ignoraLigas;
   }

   public C0713[] bB() {
      return new C0713[]{
         this.aK,
         this.aS,
         this.aR,
         this.aT,
         this.aU,
         this.aV,
         this.aW,
         this.aA,
         this.aB,
         this.aC,
         this.aD,
         this.aE,
         this.aF,
         this.aG,
         this.aX,
         this.aY,
         this.aI,
         this.aJ,
         this.IY
      };
   }

   public boolean isIgnoraEstadual() {
      return this.ignoraEstadual;
   }

   public void setIgnoraEstadual(boolean bl) {
      this.ignoraEstadual = bl;
   }

   public boolean bC() {
      return this.bI;
   }

   public void g(boolean bl) {
      this.bI = bl;
   }

   public boolean bD() {
      return this.bK;
   }

   public void h(boolean bl) {
      this.bK = bl;
   }

   public boolean bE() {
      return this.bL;
   }

   public void i(boolean bl) {
      this.bL = bl;
   }

   public boolean bF() {
      return this.bM;
   }

   public void j(boolean bl) {
      this.bM = bl;
   }

   public void f(ArrayList arrayList) {
      this.ak = arrayList;
   }

   public ArrayList bG() {
      return this.bF;
   }

   public void g(ArrayList arrayList) {
      this.bF = arrayList;
   }

   public void h(ArrayList arrayList) {
      this.an = arrayList;
   }

   public void i(ArrayList arrayList) {
      this.ao = arrayList;
   }

   public void j(ArrayList arrayList) {
      this.ap = arrayList;
   }

   public void k(ArrayList arrayList) {
      this.aq = arrayList;
   }

   public void l(ArrayList arrayList) {
      this.av = arrayList;
   }

   public void m(ArrayList arrayList) {
      this.ax = arrayList;
   }

   public void n(ArrayList arrayList) {
      this.as = arrayList;
   }

   public void o(ArrayList arrayList) {
      this.at = arrayList;
   }

   public void p(ArrayList arrayList) {
      this.au = arrayList;
   }

   public void q(ArrayList arrayList) {
      this.aw = arrayList;
   }

   public void r(ArrayList arrayList) {
      this.ay = arrayList;
   }

   public int bH() {
      return this.bg;
   }

   public void bI() {
      this.bg++;
   }

   public void w(int i) {
      this.bg = i;
   }

   public Club x(int i) {
      Object var2 = null;

      for (int var3 = 0; var3 < C0745.SR.P().size(); var3++) {
         if (((Club)C0745.SR.P().get(var3)).lk() == i) {
            return (Club)C0745.SR.P().get(var3);
         }
      }

      if (var2 == null) {
         for (int var4 = 0; var4 < C0745.SR.aG().size(); var4++) {
            if (((C0692)C0745.SR.aG().get(var4)).jn() != null && ((C0692)C0745.SR.aG().get(var4)).jn().lk() == i) {
               return ((C0692)C0745.SR.aG().get(var4)).jn();
            }
         }
      }

      return (Club)var2;
   }

   public Coach y(int i) {
      Object var2 = null;

      for (int var3 = 0; var3 < C0745.SR.L().size(); var3++) {
         if (((Coach)C0745.SR.L().get(var3)).lT() == i) {
            return (Coach)C0745.SR.L().get(var3);
         }
      }

      return (Coach)var2;
   }

   public static String z(int i) {
      Club var1 = null;

      for (int var2 = 0; var2 < C0745.SR.P().size(); var2++) {
         if (((Club)C0745.SR.P().get(var2)).lk() == i) {
            var1 = (Club)C0745.SR.P().get(var2);
            return var1.getNome();
         }
      }

      if (var1 == null) {
         for (int var5 = 0; var5 < C0745.SR.aG().size(); var5++) {
            if (((C0692)C0745.SR.aG().get(var5)).jn() != null && ((C0692)C0745.SR.aG().get(var5)).jn().lk() == i) {
               var1 = ((C0692)C0745.SR.aG().get(var5)).jn();
               return var1.getNome();
            }
         }
      }

      return "";
   }

   public ArrayList bJ() {
      return this.az;
   }

   public void s(ArrayList arrayList) {
      this.az = arrayList;
   }

   public C0950 bK() {
      return this.aL;
   }

   public void a(C0950 c0950) {
      this.aL = c0950;
   }

   public C0948 bL() {
      return this.aM;
   }

   public void a(C0948 c0948) {
      this.aM = c0948;
   }

   public C0945 bM() {
      return this.aN;
   }

   public void a(C0945 c0945) {
      this.aN = c0945;
   }

   public ArrayList bN() {
      return this.ai;
   }

   public void t(ArrayList arrayList) {
      this.ai = arrayList;
   }

   public boolean isHabilidadeIndividual() {
      return this.habilidadeIndividual;
   }

   public void setHabilidadeIndividual(boolean bl) {
      this.habilidadeIndividual = bl;
   }

   public int bO() {
      return this.bh;
   }

   public void bP() {
      this.bh++;
   }

   public void A(int i) {
      this.bh = i;
   }

   public boolean isUsarGruposReaisCopa() {
      return this.bi;
   }

   public void setUsarGruposReaisCopa(boolean bl) {
      this.bi = bl;
   }

   public C0946 bQ() {
      return this.aO;
   }

   public void a(C0946 c0946) {
      this.aO = c0946;
   }

   public C0947 bR() {
      return this.aQ;
   }

   public void a(C0947 c0947) {
      this.aQ = c0947;
   }

   public C0949 bS() {
      return this.aP;
   }

   public void a(C0949 c0949) {
      this.aP = c0949;
   }

   public boolean isJogaRegionais() {
      return this.jogaRegionais;
   }

   public void setJogaRegionais(boolean bl) {
      this.jogaRegionais = bl;
   }

   public int bT() {
      return this.bl;
   }

   public int bU() {
      this.bt++;
      return this.bt;
   }

   public C0928[] bV() {
      return this.aZ;
   }

   public void a(C0928 c0928, int i) {
      this.aZ[i] = c0928;
   }

   public boolean[] getJogaRegionaisTodos() {
      return this.jogaRegionaisTodos;
   }

   public void setJogaRegionaisTodos(boolean[] bls) {
      this.jogaRegionaisTodos = bls;
   }

   public ArrayList bW() {
      return this.ar;
   }

   public void u(ArrayList arrayList) {
      this.ar = arrayList;
   }

   public C0943 bX() {
      return this.aW;
   }

   public void a(C0943 c0943) {
      this.aW = c0943;
   }

   public ArrayList bY() {
      return this.bu;
   }

   public int bZ() {
      return this.bn;
   }

   public int ca() {
      return this.br;
   }

   public C0713 c(int i, int j) {
      C0713[] var3 = new C0713[]{this.aB, this.aC, this.aD, this.aE, this.aF, this.aG};
      C0713[] var4 = new C0713[]{this.aX, this.aY};
      C0713[] var5 = new C0713[]{this.IY};
      if (i == 4 && j < var3.length) {
         return var3[j];
      } else if (i == 6 && j < var4.length) {
         return var4[j];
      } else {
         return i == 12 && j < var5.length ? var5[j] : null;
      }
   }

   public void cb() {
      if (this.bw == null) {
         this.bw = new ArrayList();
      }

      for (int var1 = 0; var1 < this.aj.size(); var1++) {
         this.bw.add(((Club)this.aj.get(var1)).ev());
      }

      Collections.sort(this.bw, C1007.aaT);
   }

   public ArrayList cc() {
      return this.bw;
   }

   public ArrayList e(Club club) {
      ArrayList var2 = new ArrayList();

      for (int var3 = 0; var3 < this.av.size(); var3++) {
         if (((C0825)this.av.get(var3)).x().fg() != null && ((C0825)this.av.get(var3)).x().fg() == club) {
            var2.add((C0825)this.av.get(var3));
         }
      }

      return var2;
   }

   public ArrayList f(Club club) {
      ArrayList var2 = new ArrayList();

      for (int var3 = 0; var3 < this.av.size(); var3++) {
         if (((C0825)this.av.get(var3)).tP() == club) {
            var2.add((C0825)this.av.get(var3));
         }
      }

      return var2;
   }

   public C0825 e(Player player) {
      for (int var2 = 0; var2 < this.av.size(); var2++) {
         if (((C0825)this.av.get(var2)).x() == player) {
            return (C0825)this.av.get(var2);
         }
      }

      return null;
   }

   public boolean isVerLeiloes() {
      return this.verLeiloes;
   }

   public void setVerLeiloes(boolean bl) {
      this.verLeiloes = bl;
   }

   public boolean cd() {
      for (int var1 = 0; var1 < this.ak.size(); var1++) {
         if (((Club)this.ak.get(var1)).ka() != null && ((Club)this.ak.get(var1)).kb() > 0L && ((Club)this.ak.get(var1)).kw() < 35) {
            return true;
         }
      }

      return false;
   }

   public C0932 mj() {
      return this.IY;
   }

   public void a(C0932 c0932) {
      this.IY = c0932;
   }

   public int op() {
      return this.rM;
   }

   public void I(int i) {
      this.rM = i;
   }

   public void rU() {
      this.bl = 4;
      this.bm = 2;
      this.bn = 2;
      this.IZ = 3;
      this.bo = 1;
      this.bp = 1;
      this.bq = 3;
      this.br = 1;
      this.Ja = 1;
      this.Jb = 1;
   }

   public C0935 sq() {
      return this.GC;
   }

   public void a(C0935 c0935) {
      this.GC = c0935;
   }

   public C0933 yl() {
      return this.IX;
   }

   public void a(C0933 c0933) {
      this.IX = c0933;
   }

   public C0936 ym() {
      return this.GI;
   }

   public void a(C0936 c0936) {
      this.GI = c0936;
   }

   public C0934 yn() {
      return this.Gy;
   }

   public void a(C0934 c0934) {
      this.Gy = c0934;
   }

   public boolean yL() {
      return this.Jc;
   }

   public void aO(boolean bl) {
      this.Jc = bl;
   }

   public int ae(int i, int j) {
      int var3 = 0;
      if (i == 2) {
         var3 = 8;
      } else if (i == 3) {
         var3 = 9;
      } else if (i == 4) {
         var3 = j;
      } else if (i == 6) {
         if (j == 0) {
            var3 = 6;
         } else if (j == 1) {
            var3 = 7;
         }
      } else if (i == 12) {
         var3 = 14;
      } else if (i == 10) {
         var3 = 10;
      } else if (i == 8) {
         var3 = 12;
      } else if (i == 11) {
         var3 = 13;
      } else if (i == 15) {
         var3 = 15;
      } else if (i == 7) {
         if (j == 7) {
            var3 = 11;
         } else {
            var3 = 16;
         }
      } else if (i == 9) {
         var3 = 17;
      } else if (i == 14) {
         var3 = 18;
      } else if (i == 5) {
         var3 = 19;
      } else if (i == 13) {
         var3 = 20;
      }

      return var3;
   }
}
